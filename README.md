...
月次請求は生徒と対象月の組み合わせごとに1件作成し、`monthly_invoices`、`invoice_items`、`payment_records` を分離しています。管理者は「請求管理」で対象月を選び、次の順で運用します。

1. 対象月の請求下書きを生成する
2. 契約、料金、スタジオ代、警告を確認する
3. 必要なら理由付きの追加料金・値引き・調整を加える
4. 請求を確定して生徒へ公開する
5. 現金・振込・カード・外部決済等の入金を登録する

Schedulerも生成するのは下書きだけで、請求を自動確定しません。確定時に `INV-YYYYMM-連番` 形式の公開用請求番号を付けます。内部IDをそのまま請求番号として表示せず、DBの一意制約で重複を防ぎます。

### 料金スナップショットと請求内訳

月額料金は既存の `LessonPricingService` と対象月時点の適用日付き料金履歴から取得します。`regular` / `flex`、`standard` / `junior`、月回数、基本料金、フレックス加算を請求と明細へスナップショット保存します。確定済み請求は再生成・料金改定の対象外なので、後から料金表が変わっても過去の請求額は変わりません。

該当料金がない契約を0円の月額料金として確定せず、「要相談」の警告を付けます。月途中入会や月途中の契約変更も自動で日割り・按分せず、管理者確認が必要な下書きにします。警告付き請求は、管理者が警告確認を明示した場合だけ確定できます。

スタジオ代は料金表の現在値から再計算せず、対象権利月の承認済み予約に保存された `studio_fee_amount` を合計します。欠席連絡があっても契約月額料金と承認済み予約のスタジオ代は自動減額しません。キャンセル済み予約は計上しません。振替承認時には既存処理が元予約のスナップショットを振替先へ移すため、元と振替先を二重計上しません。月をまたぐ振替も `lesson_entitlement_month` を使い、元の権利月へ請求します。

手動調整は請求明細として、金額、理由、登録者、登録日時を残します。手動調整済み下書きは再生成で上書きしません。確定後は明細・金額を直接変更できません。未入金の確定請求は理由付きで取消できますが、元の明細・金額・監査履歴は保持します。

### 入金・期限・通知

1請求に複数の `payment_records` を登録でき、入金合計から未入金、一部入金、入金済み、残額を算出します。登録時は請求行と既存入金行をロックし、残額を超える入金を拒否します。支払日、金額、方法、備考、外部決済サービス名、外部参照IDを保存します。

支払期限は初期値を対象月の前月末とし、管理画面で対象月の指定日へ変更できます。期限を過ぎた未完了請求は画面上で期限超過として判定しますが、延滞金は自動加算しません。

請求確定と全額入金時は既存NotificationとQueueで生徒へ通知します。`invoice_notification_deliveries` の内容署名により同じ確定・入金完了通知の二重送信を防ぎます。通知は管理設定で個別に無効化できます。

自動生成の初期値は毎月25日02:00（Asia/Tokyo）で、ON/OFF、生成日、時刻、支払期限、通知を管理画面から変更できます。SchedulerはDB設定時刻を反映するため毎分コマンドを確認しますが、設定日時以外は何も生成しません。

```bash
./vendor/bin/sail artisan billing:generate-next-month --force
```

生徒は確定済みの自分の請求だけを閲覧でき、下書きや他生徒の請求にはアクセスできません。先生は閲覧のみ、下書き生成、調整、確定、取消、入金登録、設定変更は管理者だけが実行できます。

カード番号、有効期限、セキュリティコード、銀行口座番号、暗証番号、決済サービス秘密鍵は、請求・入金テーブルにも保存しません。将来Stripe等と連携する場合はprovider名と外部参照IDだけを保存し、provider固有処理はモデルから分離します。請求明細と入金を画面から分離しているため、将来の請求書・領収書PDFも同じ確定済みスナップショットから生成できます。
...
- `billing_settings`: 請求自動生成日・時刻、支払期限、通知設定
- `monthly_invoices`: 生徒・対象月ごとの請求、契約・料金スナップショット、確定・取消・入金集計
- `invoice_items`: 月額料金、フレックス加算、予約ごとのスタジオ代、理由付き手動調整
- `payment_records`: 1請求に対する複数入金と外部決済参照。カード・口座情報は保持しない
- `monthly_invoice_audits`: 生成、調整、確定、取消、入金の処理者・日時・変更履歴
- `invoice_notification_deliveries`: 請求確定・入金完了通知の署名と重複防止履歴
