...
- 予約枠と予約申請（承認状態、承認者、取消情報）
- SQLiteを使った自動テスト

予約申請の承認・キャンセル画面や各種変更申請は次段階で実装します。カード番号や口座番号を保存するカラムは設けていません。将来は決済代行サービスの顧客ID等だけを保持します。
...
満席判定や月回数判定の実装時は、同時承認による超過を防ぐためDBトランザクションと行ロックを使用します。
